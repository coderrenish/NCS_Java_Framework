...
public class web {
     ...
     */
    @QAFTestStep(description = "Open {0} with url {1}")
    public static void openBrowser(String app, String url) throws Exception {
        if(app.equalsIgnoreCase("Power Apps")){
            getBundle().setProperty("app.name","powerApps");
        } else {
            getBundle().setProperty("app.name","general");
        }
    ...
    public static void inputInTo(String text,String field,String page) throws Exception {
     ...
     * @param page [Page name]
     */
    @QAFTestStep(description = "Input password {0} into {1}")
     ...
     * @param page [Page name]
     */
    @QAFTestStep(description = "Click-Checkbox Field:{0} Page:{1}")
